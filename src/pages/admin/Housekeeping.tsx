...
const Housekeeping = () => {
  ...
  const fetchRooms = async () => {
    if (!tenantId) return;
    const { data } = await supabase.from("rooms").select("id, name, housekeeping_status, status").eq("tenant_id", tenantId).order("name");
  ...
  const updateStatus = async (roomId: string, newStatus: string) => {
    const { error } = await supabase.from("rooms").update({ housekeeping_status: newStatus as any }).eq("id", roomId);
    if (error) {
