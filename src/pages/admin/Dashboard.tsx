...
import {
  ...
  TrendingUp, Bot, Brain, Zap, ArrowRight, UserCheck, MessageCircleHeart,
  ShieldAlert, Loader2, Sparkles,
} from "lucide-react";
...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      ...
      const [roomsRes, bookingsRes, activeRes, dirtyRes, paymentsRes] = await Promise.all([
        supabase.from("rooms").select("id, status").eq("tenant_id", tenantId),
        supabase.from("bookings").select("id, total_amount, status, check_in, check_out").eq("tenant_id", tenantId),
        supabase.from("bookings").select("id").eq("tenant_id", tenantId).eq("status", "confirmed").lte("check_in", today).gte("check_out", today),
      ...
      const monthBookings = bookings.filter(b => b.check_in >= monthStart && b.status !== "cancelled");
      const revenue = monthBookings.reduce((sum, b) => sum + Number(b.total_amount || 0), 0);
      const outstanding = (paymentsRes.data || []).reduce((sum, b) => sum + (Number(b.total_amount || 0) - Number(b.amount_paid || 0)), 0);
      ...
      for (let i = 5; i >= 0; i--) {
        ...
        const mBookings = bookings.filter(b => b.check_in >= mStart && b.check_in <= mEnd && b.status !== "cancelled");
        revenueByMonth.push({ month: label, revenue: mBookings.reduce((s, b) => s + Number(b.total_amount || 0), 0) });
        const daysInMonth = new Date(d.getFullYear(), d.getMonth() + 1, 0).getDate();
        occupancyTrend.push({ month: label, occupancy: rooms.length > 0 ? Math.min(100, Math.round((mBookings.length / (rooms.length * daysInMonth)) * 100 * 10)) : 0 });
      }

      setStats({
        totalRooms: rooms.length, occupiedRooms: (activeRes.data || []).length,
        totalBookings: bookings.filter(b => b.status !== "cancelled").length,
        monthlyRevenue: revenue, activeGuests: (activeRes.data || []).length,
        dirtyRooms: (dirtyRes.data || []).length, outstandingPayments: outstanding,
        revenueByMonth, occupancyTrend,
      });
      setLoading(false);
    };
